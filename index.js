...
const sendRegister =  () =>{
    ...
    var errorContainerPassword = document.getElementById('error-message-password');
...
function getUsers(users){
 ...
 function alreadyExists( users, user, tipo ){
    ...
    if( !band ){
        alert(`The user ${user.name} was successfully registerd `);
        users.push(user)
        localStorage.setItem(tipo, JSON.stringify(users))
        location.reload(true);
    }else{
        alert(`The user ${user.name} already exists `);

    }
 }
 function login( users, user ){
    const band = users.find( u => u.name ===user.name && u.password === user.password );
    if(band){
